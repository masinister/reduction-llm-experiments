...
def create_cot_prompt(source_text: str, target_text: str, reduction_text: str) -> str:
    """Create a prompt for generating chain-of-thought reasoning"""
    
    prompt = f"""You are an expert in computational complexity theory and mathematical reductions. Your task is to generate a detailed chain-of-thought reasoning that explains the step-by-step process of creating the following reduction between two computational problems.
...
Now, **explain the reasoning in 3 concise (1-2 sentence) chain-of-thought steps**. In particular, format your response as follows:
...
3. **Correctness**
    - [Explain why this reduction strategy is valid. In particular,
      -- (\implies) Why are solutions to the source problem transformed into solutions to the target problem?
      -- (\impliedby) Why do solutions to the target problem correspond to solutions to the source problem?]
"""
